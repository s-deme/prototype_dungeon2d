...
namespace LanternDepths.Presentation
{
    public sealed partial class GameBootstrap
    {
        ...
        private string[] BindingNames => new[] { L("上", "Up"), L("左", "Left"), L("下", "Down"), L("右／持ち物を置く", "Right / Drop"), L("左上", "Northwest"), L("右上／装備", "Northeast / Equip"), L("左下", "Southwest"), L("右下", "Southeast"), L("待機", "Wait"), L("拾う", "Pick up"), L("持ち物", "Pack"), L("階段／決定", "Stairs / Confirm"), L("使う", "Use"), L("演出切替", "Animation"), L("再出発", "Restart"), L("敵情報", "Inspect") };
        ...
        private void OpenHelp()
        {
            ...
            utility.Text(L("目標：5階で守護者を倒し、階段 > から残り火を回収。\n\n① 矢印・WASD・テンキー・左スティックで移動。Q E Z Cは斜め移動。敵のマスに進むと攻撃。\n② ! 薬・/ 武器・] 防具・? 退避・* 爆薬の上で「拾う」。持ち物から使用・装備できる。\n③ 行動が成功すると敵も1回動く。持ち物を見る・敵を調べる・メニューを開く操作では進まない。\n④ 射手 r は4マス先まで射撃。壁を使って接近。守護者 B の強打予告を見たら離れよう。\n⑤ > の上で階段操作。行動ごとに自動保存され、再起動で続きから遊べる。\n\n敵をクリック、または敵情報キー／右スティック押し込みでHPと特性を確認。\n持ち物：上下で選択、U/Aで使用、E/Xで装備、D/RBで置く。\nメニュー：上下で選択、Enter/Aで決定、Esc/Bで戻る。\nコントローラー：A 待機、X 拾う、Y 持ち物、LB 階段、Start メニュー。\n\n現在のキー割り当て：", "Goal: defeat the guardian on floor 5 and claim the ember at >.\n\n1. Move with arrows, WASD, numpad or stick; Q E Z C move diagonally. Bump into an enemy to attack.\n2. Pick up ! tonics, / weapons, ] armor, ? retreat runes and * blast flasks. Use or equip from your pack.\n3. Successful actions advance enemies one turn. Browsing, inspecting and menus are free.\n4. Archers r shoot up to 4 tiles; approach behind walls. Step away when the guardian B warns of a strike.\n5. Use stairs > to descend. Each action autosaves; relaunch to continue.\n\nClick an enemy or Inspect / right stick click for health and traits.\nPack: arrows select, U/A use, E/X equip, D/RB drop.\nMenu: arrows select, Enter/A confirm, Esc/B back.\nController: A wait, X pick up, Y pack, LB stairs, Start menu.\n\nCurrent bindings:") + "\n" + string.Join("   ·   ", BindingNames.Select((n, i) => n + " " + (KeyCode)preferences.keys[i])));
            ApplyPreferences();
        }
    }
}
